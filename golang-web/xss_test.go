...
func TestTemplateAutoEscape(t *testing.T) {
	...
	TemplateAutoEscape(record, request)
...
func TemplateAutoEscapeDisabled(writer http.ResponseWriter, request *http.Request) {
	myTemplates.ExecuteTemplate(writer, "post.gohtml", map[string]any{
		"Title": "Template Auto Escape",
		"Body":  template.HTML("<h2>Ini adalah body</h2>"),
	})
}
...
func TestTemplateAutoEscapeDisabled(t *testing.T) {
	...
	TemplateAutoEscapeDisabled(record, request)
...
func TestTemplateXSS(t *testing.T) {
	request := httptest.NewRequest(http.MethodGet, "http://localhost:8080?body=<script>alert('anda%20dihack')</script>", nil)
	record := httptest.NewRecorder()

	TemplateXSS(record, request)
